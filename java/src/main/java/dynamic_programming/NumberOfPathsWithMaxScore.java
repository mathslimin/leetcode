...
/**
 * Created  on 13/04/2021 You are given a square board of characters. You can
            ...
            for (int j = board.get(i).length() - 1; j >= 0; j--) {
                ...
                if (curr != 'X') {
                    ...
                    for (int k = 0; k < 3; k++) {
                        int newR = i + R[k];
                        int newC = j + C[k];
                        if (newR < board.size() && newC < board.get(0).length() &&
                            board.get(newR).charAt(newC) != 'X' && N[newR][newC] != 0) {
