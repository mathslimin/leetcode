...
/**
 * Created  on 27/01/2018. In a N x N grid representing a field of cherries,
...
public class CherryPickup {
    ...
    private int dp(int N, int r1, int c1, int c2, int[][][] DP, int[][] grid) {
        ...
        if (r1 >= N || c1 >= N || c2 >= N || r2 >= N || grid[r1][c1] == -1 || grid[r2][c2] == -1)
            ...
            // down),
            // (P1 down, P2 down)
            max += Math.max(Math.max(Math.max(dp(N, r1 + 1, c1, c2, DP, grid), dp(N, r1 + 1, c1, c2 + 1, DP, grid)),
