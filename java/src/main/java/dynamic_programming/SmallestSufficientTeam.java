...
/**
 * Created  on 04/06/2020 In a project, you have a list of required skills
    ...
    private int dp(int i, int skill, int[][] DP) {
        ...
        if (skill == allSkills)
            ...
            if (Math.min(with, withOut) == Integer.MAX_VALUE) {
                ...
                DP[skill][i] = ((with & MAX_SKILLS) < (withOut & MAX_SKILLS)) ? ((with & MAX_SKILLS) | CHOOSE_PEOPLE)
                                                                              : (withOut & MAX_SKILLS);
            return DP[skill][i];
        }
    }
