...
/**
 * Created  on 27/03/2017. Given a binary tree, return the values of its
...
public class BoundaryOfBinaryTree {
    ...
    private void leftShoulder(TreeNode node, List<Integer> list) {
        ...
        if (node.left != null)
            leftShoulder(node.left, list);
        else if (node.right != null)
            leftShoulder(node.right, list);
    }
    ...
    private void rightShoulder(TreeNode node, List<Integer> list) {
        ...
        if (node.right != null)
            rightShoulder(node.right, list);
        else if (node.left != null)
            rightShoulder(node.left, list);
    }
