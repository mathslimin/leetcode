...
/**
 * Created  on 25/05/2019 Remember the story of Little Match Girl? By now, you
...
public class MatchsticksToSquare {
    ...
    private void generate(int i, int[] nums, List<Pair> subList, List<List<Pair>> result, int sum, int expected) {
        if (i >= nums.length) {
            if (sum == expected) {
