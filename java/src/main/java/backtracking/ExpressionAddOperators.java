...
/**
 * Created  on 05/12/2017. Given a string that contains only digits 0-9 and a
...
public class ExpressionAddOperators {
    ...
    private void backTrack(String exp, List<String> list, int curr, String num, int target, long total, long prod) {
        if (curr == num.length()) {
            if (total == target) {
                list.add(exp);
            }
