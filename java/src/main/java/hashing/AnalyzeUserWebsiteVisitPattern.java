...
/**
 * Created  on 23/10/2019 We are given some website visits: the user with name
...
public class AnalyzeUserWebsiteVisitPattern {
    ...
    public static void main(String[] args) {
        ...
        List<String> result = new AnalyzeUserWebsiteVisitPattern().mostVisitedPattern(userName, time, website);
    ...
    public List<String> mostVisitedPattern(String[] username, int[] timestamp, String[] website) {
                ...
                if (username[i].equals(username[j])) {
                    for (int k = j + 1; k < username.length; k++) {
                        if (username[i].equals(username[j]) && username[j].equals(username[k])) {
                            List<WebsiteTime> visits = Arrays.asList(new WebsiteTime(website[i], timestamp[i]),
                            ...
                            String concatinatedWebsite =
                                String.join("-", visits.get(0).website, visits.get(1).website, visits.get(2).website);
        ...
        visitCounts.sort(Comparator.comparingInt(VisitCount::getCount).reversed().thenComparing(VisitCount::getSite));
